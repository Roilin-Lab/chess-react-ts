...
import { replaceMoveToUnicodeIcon } from "../chess/utils";
  ...
  return (
    <>
      <PanelContainer>
        {chess.isGameOver() ? (
          chess.isStalemate() ? (
            <BoardInfo>Пат!</BoardInfo>
          ) : (
            <BoardInfo>
              Победа {chess.turn() === "w" ? "чёрных" : "белых"}
            </BoardInfo>
          )
        ) : (
        ...
        <HistoryList>
          {chess.history({ verbose: true }).map((move, index) => {
            const moveWithIcon = replaceMoveToUnicodeIcon(move);
            return (
              <>
                {index % 2 === 0 ? (
